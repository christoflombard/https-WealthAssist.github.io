...
export default function FluidCursorWebGPU() {
...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    // Gold color shader - modified to use gold color
...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    let col_start = vec3(0.831, 0.686, 0.216);
...
`;
    ...
    class DynamicBuffer {
      ...
      constructor({
        ...
        this.w = w;
        this.h = h;
        this.buffers = new Array(dims).fill(null).map(() =>
          device.createBuffer({
    ...
    class Uniform {
      ...
      constructor(name: string, { size, value }: { size?: number; value?: number | number[] } = {}) {
        this.name = name;
        ...
        if (this.size === 1) {
          if ((settings as any)[name] == null) {
            (settings as any)[name] = value ?? 0;
        ...
        if (this.size === 1 || value != null) {
          ...
          const sourceValue = value ?? [(settings as any)[this.name]];
      ...
      setValue(value: number) {
        (settings as any)[this.name] = value;
        this.needsUpdate = true;
      }

      update(queue: GPUQueue, value?: number[]) {
        if (this.needsUpdate || this.alwaysUpdate || value != null) {
          if (typeof this.needsUpdate !== 'boolean') value = this.needsUpdate;
          queue.writeBuffer(
            this.buffer,
            0,
            new Float32Array(value ?? [parseFloat((settings as any)[this.name])]),
            0,
            this.size
          );
          this.needsUpdate = false;
    ...
    class RenderProgram {
      ...
      dispatch(commandEncoder: GPUCommandEncoder) {
        (this.renderPassDescriptor.colorAttachments as any)[0].view = context.getCurrentTexture().createView();
    ...
    function initUniforms() {
      ...
      containFluid = new Uniform('contain_fluid', { value: settings.contain_fluid ? 1 : 0 });
    ...
    function refreshSizes() {
      ...
      globalUniforms.gridSize.needsUpdate = [
        ...
        settings.dx, settings.rdx, settings.dyeRdx
      ];
    }
    ...
    async function main() {
      ...
      canvas!.addEventListener('touchstart', (e) => {
        e.preventDefault();
        handlePointerMove(e);
        mouseInfos.last = mouseInfos.current ? [...mouseInfos.current] : null;
      });
